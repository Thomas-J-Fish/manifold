...
Tab** — opens a picker grouping all nineteen modes under five subjects, with a line of description each.
...
### The nineteen modes, in five subjects

Modes are filed under **Mathematics**, **Physics**, **Chemistry**, **Data &
Computation** and **Finance** — the two levels the new-tab picker and the examples dialog both
...
| **Loan Interest** | A debt marched month by month: the remaining capital, the interest charged each month, and what it all adds up to. Rates are piecewise, so a fixed period ending partway through is the normal case rather than an awkward one — and because the schedule is computed rather than looked up, the annuity formula's assumption of a single rate never has to hold. Interest can be paid each month, in which case the balance falls by exactly the capital payment and the rate decides the cost rather than the term; or added to the debt, in which case it compounds and a payment below the monthly interest never clears it at all — which the mode says outright rather than drawing as a slow decline. Either the payment or the term is the input and the other is the answer: fix the payment and read the term off the chart, or say fourteen years and ten months and the mode solves for the capital that needs — by bisection on the schedule itself, so a piecewise rate and a one-off overpayment are handled where no closed form inverts. Sliders for the balance, the payment and each rate, each with a typed box beside it, since fifteen percentage points across a few hundred pixels cannot be dragged to 1.09% on purpose. The term, the total interest and the jump at a rate change all update live. |
...
**Help → Load an Example** opens sixty-seven worked examples, grouped by subject and then by mode,
...
src/
  core/
    ...
    finance/
      loan.ts          amortisation schedules, piecewise rates, overpayments,
                       and the inverse: the payment that clears a debt in a given term
    chemistry/
